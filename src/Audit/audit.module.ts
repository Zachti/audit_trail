...
import { AuditOptions, AUDIT_OPTIONS , AuditModuleAsyncOptions } from './audit.interfaces';
...
export class AuditModule {
    ...
    private static createAsyncProviders(options: AuditModuleAsyncOptions): Provider[] {
        ...
        if (options.useClass || options.useExisting) {
            return [this.createAsyncOptionsProvider(options), options.useClass || options.useExisting];
        }
    ...
    private static createAsyncOptionsProvider(options: AuditModuleAsyncOptions): Provider {
        if (options.useFactory) {
            return {
                provide: AUDIT_OPTIONS,
        ...
        throw new Error('Invalid configuration for AuditModule. Provide useFactory or useClass in the options.');
    }
}
